...
public class TerraLetras extends LyricProvider {	
	@Override
	public String decodeLyricHtml(String html, Lyric actualLyric) throws LyricNotFoundException {
		String lyric;
		...
		if (header.equalsIgnoreCase("Música não encontrada")) {
			...
			html = html.substring(html.indexOf(endingElement, 0) + endingElement.length());
		}

		int indexOfStartingElement = html.indexOf(startingElement);
		int[] lyricsRange = new int[] { indexOfStartingElement + startingElement.length(),
	...
	public String encodeToUrl(String target) {
		return target.trim().replace(' ', '+').replace("%26", "&").toLowerCase();
	}
}
